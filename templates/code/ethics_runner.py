...
def is_moral_theory_runner(theory, context):
    '''
    Takes an ethical theory and an iterable of arguments that theories
    of this type should accept... wait, no!
    ...
    if theory['is_kantian']:
        if context['benign_intent'] and context['foreseeable_resulting_harm'] < 0.1:
            # Add checks for more sophisticated theories here, and add the
            # requisite properties to the theory and context so that we
            # have what we need in order to perform those checks.
            return True
        else:
            return False

    # TODO(elimisteve): Add, uh, way more checks here
